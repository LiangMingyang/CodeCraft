...
(function() {
  ...
  exports.getSubmission = function(req, res) {
    ...
    Submission = global.db.models.submission;
    return global.db.Promise.resolve().then(function() {
      return global.redis.lindex('_judge_queue', 0);
    }).then(function(submission_id) {
      if (!submission_id || parseInt(req.params.submissionId) < parseInt(submission_id)) {
        return global.myUtils.findContest(req.session.user, req.params.contestId).then(function(contest) {
          if (!contest && !req.session.user) {
            throw new global.myErrors.UnknownUser();
          }
          if (!contest) {
          ...
          return res.json(submission.get({
            plain: true
          }));
        });
      } else {
