...
(function() {
  var FS, HOME_PAGE, INDEX_PAGE, LOGIN_PAGE, fs, path, sequelize;
  ...
  exports.postTask = function(req, res) {
    ...
    return global.myUtils.checkJudge(req.body.judge).then(function() {
      ...
      return fs.readFilePromised(path.join(global.myUtils.getStaticProblem(submission.problem_id), 'manifest.json'));
    }).then(function(manifest_str) {
      currentSubmission.dataValues.manifest = JSON.parse(manifest_str);
      return res.json(currentSubmission);
    })["catch"](global.myErrors.UnknownSubmission, function() {
      return res.end();
    })["catch"](function(err) {
      console.log(err);
      return res.end();
  ...
  exports.postFile = function(req, res) {
    return global.myUtils.checkJudge(req.body.judge).then(function() {
      ...
      return download(path.join(global.myUtils.getStaticProblem(problemID), filename), filename);
    })["catch"](function(err) {
      console.log(err);
      return res.end();
    });
