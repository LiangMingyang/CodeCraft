...
(function() {
  ...
  notify = function(message, type, delay = -1) {
  ...
  angular.module('contest-factory', []).factory('Submission', function($http, $timeout) {
    ...
    Poller = function() {
      var id_list, j, len, sub, submission_id;
      if (Sub.first_time) {
        ...
        for (j = 0, len = id_list.length; j < len; j++) {
          submission_id = id_list[j];
          Sub._submission_id = Math.max(submission_id, Sub._submission_id);
        }
        if (Sub._submission_id > 0) {
          return $http.get(`/api/contests/${Sub.contestId}/submission/${Sub._submission_id}`).then(function(res) {
            var i, k, len1, ref;
            if (res.data !== '') {
              ref = Sub.data;
